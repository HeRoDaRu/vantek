...
export interface AppConfig {
  empresa: { nombre: string; cif: string; direccion: string; telefono: string; email: string; logo: string };
  documentos: {
    iva: number; margen_defecto: number; max_versiones: number;
    numeracion_factura: { contador: number; anno: number; reinicio_pendiente: boolean };
  };
  conceptos_defecto: { id: string; label: string; precio_hora: number; unidad: string }[];
  sistema: { email_errores: string; actualizacion: { hora_inicio: string; hora_fin: string; inactividad_minutos: number } };
}
